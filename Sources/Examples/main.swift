...
struct FormView: View {
    ...
    /// The fuzzy-picker half of the demo: the "Favorite?" TextField narrows
    /// this list, and the SelectList below it clamps its selection as the
    /// options shrink.
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 0) {
                ...
                Text("Favorite? ").bold()
                Text("› ").foregroundColor(.brightBlack)
                TextField("Type to filter plans", text: $planFilter)
            }
            HStack(spacing: 0) {
                Text("  ")
                SelectList(filteredPlans, selection: $favoriteIndex, visibleRows: 6)
            }
            if phase == 0 {
                HStack(spacing: 0) {
                    Text("  ")
